# FILE: gateway_server.py (Cập nhật để hỗ trợ chuyển đổi view tức thì và lọc theo đối tượng)
...
def startup_event():
    ...
    try:
        ...
        # Create a 'name_stem' column by removing the extension
...
# --- Helper for Temporal Checklist Logic ---
def is_temporal_sequence_valid(sequence: Dict, filters: ObjectFilters) -> bool:
    """
    Checks if a temporal sequence meets all filter conditions across its keyframes.
    """
            ...
            if not frame_positions.is_empty():
                for i, p_box in enumerate(filters.positioning.boxes):
                    ...
                    if key in checklist and p_box.label in frame_positions['object']:
                        # Use pl.when().then().otherwise() for universal Polars compatibility
                        user_x_min_lit = pl.lit(p_box.box[0] * IMAGE_WIDTH)
                        user_y_min_lit = pl.lit(p_box.box[1] * IMAGE_HEIGHT)
                        user_x_max_lit = pl.lit(p_box.box[2] * IMAGE_WIDTH)
                        user_y_max_lit = pl.lit(p_box.box[3] * IMAGE_HEIGHT)
...
def get_valid_filepaths_for_strict_search(all_filepaths: set, filters: ObjectFilters) -> set:
    ...
    if filters.counting and OBJECT_COUNTS_DF is not None and filters.counting.conditions:
        ...
        for obj, cond_str in filters.counting.conditions.items():
            op, val = parse_condition(cond_str)
            if op and val is not None and obj in df_subset.columns: 
...
def process_and_cluster_results(results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for video_id, shots in shots_by_video.items():
        ...
        for shot in shots: 
            shots_by_shot_id[shot['shot_id_int']].append(shot)
...
def add_image_urls(data: List[Dict[str, Any]], base_url: str):
    for item in data:
        # Xử lý cho Simple/OCR/ASR search và Temporal (non-clustered view)
        ...
        # Xử lý cho Temporal (clustered view)
        for cluster in item.get('clusters', []):
            for shot in cluster.get('shots', []):
                if shot.get('filepath') and 'url' not in shot:
                    shot['url'] = f"{base_url}images/{base64.urlsafe_b64encode(shot['filepath'].encode('utf-8')).decode('utf-8')}"
            
            if (best_shot_in_cluster := cluster.get('best_shot')) and best_shot_in_cluster.get('filepath') and 'url' not in best_shot_in_cluster:
...
async def search_unified(request: Request, search_data: str = Form(...), query_image: Optional[UploadFile] = File(None)):
    ...
    if search_data_model.filters:
        ...
        # Rebuild clusters, chỉ giữ lại các shots hợp lệ
        final_results = []
...
async def temporal_search(request_data: TemporalSearchRequest, request: Request):
    ...
    if cluster_mode:
        ...
        # Xử lý kết quả cuối cùng để chứa CẢ HAI chế độ xem
        processed_sequences = []
        for cluster_seq in all_valid_cluster_sequences:
            if not cluster_seq: 
                continue
            
            # Dữ liệu cho chế độ xem không-cluster (sequence của các best shots)
            ...
            # Lấy video_id từ cluster đầu tiên
            video_id = cluster_seq[0].get('video_id', 'N/A')

            processed_sequences.append({
                "average_rrf_score": avg_score,
                "clusters": cluster_seq,  # Dữ liệu cho chế độ xem CLUSTERED
                "shots": shot_sequence,   # Dữ liệu cho chế độ xem NON-CLUSTERED
                "video_id": video_id
            })

        sequences_to_filter = sorted(processed_sequences, key=lambda x: x['average_rrf_score'], reverse=True)
    
    # Áp dụng bộ lọc đối tượng
    if filters:
...
async def ocr_search(request_data: OcrSearchRequest, request: Request):
    ...
    print(f"OCR Search | Enhance: {request_data.enhance}, Expand: {request_data.expand}, Filters: {request_data.filters}")
    base_query = translate_text(request_data.query)
    queries_to_process = expanding(base_query) if request_data.expand else [base_query]
    ...
    # Áp dụng bộ lọc đối tượng nếu có
...
async def asr_search(request_data: AsrSearchRequest, request: Request):
    ...
    print(f"ASR Search | Enhance: {request_data.enhance}, Expand: {request_data.expand}, Filters: {request_data.filters}")
    base_query = translate_text(request_data.query)
    queries_to_process = expanding(base_query) if request_data.expand else [base_query]
    queries_to_search = [enhancing(q) for q in queries_to_process] if request_data.enhance else queries_to_process
